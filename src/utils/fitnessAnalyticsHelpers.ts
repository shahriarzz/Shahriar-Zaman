 ...
 */
export function mapTargetToCategory(targetStr: string): MuscleCategory {
...
/**
 * Resolves exercise metadata using exerciseDefinitionId as canonical identifier
 */
export function getResolvedExerciseMeta(
  exerciseDefinitionId: string,
  defsMap: Map<string, ExerciseDefinition>
): ResolvedExerciseMeta {
  const def = defsMap.get(exerciseDefinitionId);
  if (def) {
    return {
      id: def.id,
      name: def.name,
  ...
  // Fallback for custom or deleted IDs that are not present in current definitions library
  return {
    id: exerciseDefinitionId,
    name: exerciseDefinitionId || 'Exercise',
    target: 'General',
    category: 'Core',
    tags: []
  };
  ...
  workouts.forEach(wo => {
    (wo.exercises || []).forEach((ex: WorkoutExercise) => {
      if (ex.tags?.includes('priority') && ex.exerciseDefinitionId && !seenIds.has(ex.exerciseDefinitionId)) {
        const meta = getResolvedExerciseMeta(ex.exerciseDefinitionId, defsMap);
        seenIds.add(meta.id);
